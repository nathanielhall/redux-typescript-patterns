...
import {
  fromActions as fromNotificationActions,
  NotificationType
...
export const addTodo: Middleware = ({dispatch}: MiddlewareAPI) => (
  ...
  switch (action.type) {
    ...
    case fromTodosActions.ADD_TODO_ONSUCCESS: {
      ...
      dispatch(fromTodosActions.Actions.setTodo(action.payload));
...
export const deleteTodo: Middleware = ({dispatch}: MiddlewareAPI) => (
  ...
  switch (action.type) {
    ...
    case fromTodosActions.DELETE_TODO_ONSUCCESS: {
      ...
      dispatch(fromTodosActions.Actions.setDeletedTodo(_id));

      dispatch(
        fromNotificationActions.Actions.setNotification(
          `Deleted todo id ${_id.toString()}`,
          NotificationType.SUCCESS,
          MODULE
        )
      );
...
export const todosMdl = [fetchTodoItems, deleteTodoItems, addTodo, deleteTodo];
